...
# W1: WorkSpace de reference : series de l'serie_ipi jusqu'en septembre 2021
ch_1 <- "./WS/WS_ref.xml"

# Pour les 3 prochains WorkSpaces, on a ajouté 3 points (d'oct. à dec. 2021)

# W2 : Refresh current --> on garde le même modèle
ch_2 <- "./WS/WS_fixed_model.xml"
# W3 : Refresh en last AO (v 2.2.3 de JD + )
ch_3 <- "./WS/WS_AO_current_approach.xml"
# W4 : Refresh concurrent
ch_4 <- "./WS/WS_concurrent.xml"
...
summary_ws <- data.frame(
    num_ws = 1:4,
    name = c("reference", "fixed_model", "AO_current_approach", "concurrent"),
    path = c(ch_1, ch_2, ch_3, ch_4),
    id = c("ref", "cur", "ao", "conc")
)
...
realises_ws_df <- list_ws$ref$rea %>% 
    merge(y = list_ws$cur$rea, by = "date", all = TRUE) %>% 
    merge(y = list_ws$ao$rea, by = "date", all = TRUE) %>% 
    merge(y = list_ws$conc$rea, by = "date", all = TRUE)

# Les séries de forecast (prévision)
prevision_ws_df <- list_ws$ref$prev %>% 
    merge(list_ws$cur$prev, by = "date", all = TRUE) %>% 
    merge(list_ws$ao$prev, by = "date", all = TRUE) %>% 
    merge(list_ws$conc$prev, by = "date", all = TRUE)

# Les séries étendues réalisées puis forecast
extended_ws_df <- list_ws$ref$extended %>% 
    merge(list_ws$cur$extended, by = "date", all = TRUE) %>% 
    merge(list_ws$ao$extended, by = "date", all = TRUE) %>% 
    merge(list_ws$conc$extended, by = "date", all = TRUE)
...
### Création de modèles ARIMA --------------------------------------------------

# Concernant les ordres et coefficients du modèle Reg-ARIMA, 
# On remarque que ce sont les mêmes pour ref, cur et ao
# Le modèle concurrent a des coefficients différents mais les mêmes ordres
# On prend le modèle de référence.

# On prend y du modele current
y <- list_ws$cur$modele$final$series[, "y"]

# On prend y_lin du modele current
y_lin <- list_ws$cur$modele$regarima$model$effects[, "y_lin"]

ordres <- list_ws$ref$modele$regarima$specification$arima$specification[, 
                paste0("arima.", c("p", "d", "q")), drop = TRUE] %>% 
    as.numeric()
ordres_s <- list_ws$ref$modele$regarima$specification$arima$specification[, 
                paste0("arima.b", c("p", "d", "q")), drop = TRUE] %>% 
    as.numeric()
coeffs <- list_ws$ref$modele$regarima$arima.coefficients[, "Estimate"]

# But : Comparer les modèles ARIMA selon que l'on considère :
#   - Estimation 1.a : les ordres + coefficients du modèle SA
#   - Estimation 1.b : uniquement les ordres et on estime alors les coefficients
#   - Estimation 1.c : rien du tout ! On repart de y linéarisé
#   - Estimation 2 : rien du tout ! On repart de y et on ré-estime par regarima

# Estimation 1 : utliser y_lin -------------------------------------------------

arima_base_1a <- arima(
    x = y_lin, order = ordres, seasonal = ordres_s, fixed = coeffs)
arima_forecast_1a <- forecast::Arima(
    y = y_lin, order = ordres, seasonal = ordres_s, fixed = coeffs)

arima_base_1b <- arima(
    x = y_lin, order = ordres, seasonal = ordres_s)
arima_forecast_1b <- forecast::Arima(
    y = y_lin, order = ordres, seasonal = ordres_s)

arima_base_1c <- arima(x = y_lin)
arima_forecast_1c <- forecast::auto.arima(y = y_lin)

# Estimation 2 : Utiliser le module reg-ARIMA sur la série brute y -------------

regarima_rjdemetra_2 <- RJDemetra::regarima(
    series = y, 
    spec = regarima_spec_x13(spec = "RG5c"))

new_ordres <- regarima_rjdemetra_2$specification$arima$specification[, 
        paste0("arima.", c("p", "d", "q")), drop = TRUE] %>% as.numeric()
new_ordres_s <- regarima_rjdemetra_2$specification$arima$specification[, 
        paste0("arima.b", c("p", "d", "q")), drop = TRUE] %>% as.numeric()
new_coeffs <- regarima_rjdemetra_2$arima.coefficients[, "Estimate"]
new_y_lin <- regarima_rjdemetra_2$model$effects[, "y_lin"]

arima_base_2 <- arima(
    x = new_y_lin, order = new_ordres, 
    seasonal = new_ordres_s, fixed = new_coeffs)
arima_forecast_2 <- forecast::Arima(
    y = new_y_lin, order = new_ordres, seasonal = new_ordres_s, 
    fixed = new_coeffs)

# Affichage des graphiques -----------------------------------------------------

dygraphs::dygraph(cbind(arima_forecast_1a$fitted, 
                        arima_forecast_1b$fitted, 
                        arima_forecast_1c$fitted, 
                        arima_forecast_2$fitted))
